...
export function watchStorySpecifiers(
  ...
  async function onChangeOrRemove(watchpackPath: Path, removed: boolean) {
    ...
    const importPath = watchpackPath.startsWith(".")
      ? watchpackPath
      : `./${watchpackPath}`;
  ...
  wp.on("change", async (filePath: Path, mtime: Date, _explanation: string) => {
  ...
  wp.on("remove", async (filePath: Path, _explanation: string) => {
