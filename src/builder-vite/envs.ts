 ...
 */
export function stringifyProcessEnvs(
  raw: Builder_EnvsRaw,
    ...
    (acc: Builder_EnvsRaw, [key, value]) => {
      ...
      if (
        ...
        (Array.isArray(envPrefix) &&
          !!envPrefix.find((prefix) => key.startsWith(prefix))) ||
...
export async function sanitizeEnvVars(options: Options, config: ViteConfig) {
  ...
  // 現状だとenvsRawがundefinedなので、一旦、undefinedの時はreturnするように
  if (!envsRaw) {
    return {
      ...config,
      define,
