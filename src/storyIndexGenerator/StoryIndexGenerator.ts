...
class IndexingError extends Error {
  ...
  pathsString() {
    ...
    return `${this.importPaths.map((importPath) => importPath).join(",")}`;
  }
...
export class StoryIndexGenerator {
  ...
  private specifierToCache: Map<
    NormalizedStoriesSpecifier,
    SpecifierStoriesCache
  >;
  ...
  async initialize() {
    ...
    const specifiersAndCaches = await Promise.all(
      this.specifiers.map(async (specifier) => {
        ...
        if (files.length === 0) {
          console.warn(
            `No story files found for the specified pattern: ${path.join(
              specifier.directory,
              specifier.files
            )}`
          );
        ...
        files.sort().forEach((absolutePath: Path) => {
          ...
          if (ext === ".storyshot") {
            ...
            console.info(`Skipping ${ext} file ${relativePath}`);
            return;
          }
      ...
      this.specifiers.map(async (specifier) => {
        ...
        return Promise.all(
          Object.keys(entry || {}).map(async (absolutePath) => {
            ...
            try {
              entry[absolutePath] = await updater(
                specifier,
                absolutePath,
                entry[absolutePath]
              );
  ...
  async ensureExtracted(): Promise<(IndexEntry | ErrorEntry)[]> {
    ...
    await this.updateExtracted(async (specifier, absolutePath) =>
      this.isDocsMdx(absolutePath)
        ? false
        : this.extractStories(specifier, absolutePath)
    );
    ...
    return this.specifiers.flatMap((specifier) => {
      ...
      return Object.values(cache || {}).flatMap(
        (entry): (IndexEntry | ErrorEntry)[] => {
          if (!entry) return [];
  ...
  async extractStories(
    ...
    const importPath = normalizeStoryPath(relativePath);
    const makeTitle = (userTitle?: string) => {
      return (
        userOrAutoTitleFromSpecifier(importPath, specifier, userTitle) || ""
      );
    };
  ...
  async extractDocs(specifier: NormalizedStoriesSpecifier, absolutePath: Path) {
    ...
    try {
      ...
      if (result.of) {
        ...
        dependencies.forEach((dep) => {
          if (dep.entries.length > 0) {
            ...
            if (
              path
                .normalize(
                  path.resolve(this.options.workingDir, first.importPath)
                )
        ...
        if (!csfEntry)
          throw new Error(
            `Could not find CSF file at path "${result.of}" referenced by \`of={}\` in docs file "${relativePath}".
      ...
      const title =
        // @ts-ignore
        csfEntry?.title ||
        userOrAutoTitleFromSpecifier(importPath, specifier, result.title);
      const { defaultName } = this.options.docs;
      const name =
        result.name ||
        // @ts-ignore
        (csfEntry
          ? autoName(importPath, csfEntry.importPath, defaultName || "")
          : defaultName);
      ...
      const docsEntry: DocsCacheEntry = {
        ...
        name: name || "",
        importPath,
        storiesImports: sortedDependencies.map(
          (dep) => dep.entries[0].importPath
        ),
        type: "docs",
        tags: [
          ...(result.tags || []),
      ...
      if (err.source?.match(/mdast-util-mdx-jsx/g)) {
        console.warn(
          `💡 This seems to be an MDX2 syntax error. Please refer to the MDX section in the following resource for assistance on how to fix this: 
  ...
  chooseDuplicate(firstEntry: IndexEntry, secondEntry: IndexEntry): IndexEntry {
    ...
    if (secondEntry.type === "story") {
      ...
      isMdxEntry(secondEntry) &&
      firstEntry.type === "docs" &&
      !isMdxEntry(firstEntry)
    ) {
      firstIsBetter = false;
    }
    ...
    const changeDocsName =
      'Use `<Meta of={} name="Other Name">` to distinguish them.';
    ...
    if (betterEntry.type === "story") {
      ...
      if (
        ...
        this.options.docs.autodocs !== true
      )
        throw new IndexingError(
  ...
  async getIndex() {
    ...
    try {
      const errorEntries = storiesList.filter(
        (entry) => entry.type === "error"
      );
      if (errorEntries.length)
        throw new Error(
          "errorEntries.map((entry) => (entry as ErrorEntry).err)ってエラーをthrowする"
        );
      ...
      if (duplicateErrors.length)
        throw new Error("duplicateErrorsってエラーをthrowする");
        ...
        compat = Object.entries(sorted).reduce((acc, entry) => {
          ...
          acc[id] = {
            ...
            parameters: {
              __id: story.id,
              docsOnly:
                titleToStoryCount[story.title] === 1 && story.name === "Page",
  ...
  storyFileNames() {
    return Array.from(this.specifierToCache.values()).flatMap((r) =>
      Object.keys(r)
    );
  }
}
